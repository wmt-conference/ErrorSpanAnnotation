...
df = AnnotationLoader(refresh_cache=False).get_view(["WMT-MQM", "MQM-1", "ESA-1", "ESAAI-1", "ESA-2", "ESAAI-2"], only_overlap=False).dropna()
import scipy.stats
...
for protocol in ["MQM", "ESA", "ESAAI"]:
    if protocol == "MQM":
        corr = scipy.stats.spearmanr(df[protocol + "-1_score"], df["WMT-MQM_score"])[0]
    else:
    ...
    if protocol == "MQM":
        scores_2 = [mqm_like_score(x) for x in df["WMT-MQM_error_spans"]]
    else:
...
df = AnnotationLoader(refresh_cache=False).get_view(["MQM-1", "ESA-1", "ESAAI-1", "ESA-IAA", "ESAAI-IAA"], only_overlap=True).dropna()
