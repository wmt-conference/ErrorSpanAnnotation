...
anno_gemba = AppraiseAnnotations(f"campaign-ruction-rc5/240315rc5GEMBA.scores.csv", "GEMBA").generate_scores().df


to_average_users = []
for login in anno_gemba.login.unique():
    df = anno_gemba[anno_gemba["login"] == login]
...
for user_i, user in enumerate(to_average_users):
    print(
        f"{user['time']:.1f}s",
        f"{user['span_orig']:.1f}",
        f"{user['span_kept']:.1f}",
